...
class IDManager {
public:
	IDManager(IDOrder order = IDOrder::RANDOM)
		: IDManager(0, order) {
	}

	IDManager(ID_TYPE startCount, IDOrder order = IDOrder::RANDOM) {
	...
	*/
	ID_TYPE GetNewUniqueIdentifier() {
		switch (m_order) {
		case IDOrder::RANDOM: return GetNewUniqueIdentifierRandom();
...
private:
	...
	IDOrder m_order = IDOrder::RANDOM;
...
inline std::string FormatUtils::toString<IDOrder>(IDOrder order) {
	switch (order) {
	case IDOrder::RANDOM:		return "Random";
